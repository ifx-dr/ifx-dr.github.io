...
def reformat_properties(soup, property):
    new_soup = BeautifulSoup("", 'html.parser')
    for div in soup.find_all("div", class_="entity"):
        h3 = div.find('h3')
        if h3 and h3.find('sup', text=property):  # Check if h3 and 'sup' with text 'op' exist
            ...
            try:
                ...
                for a_tag in dl.find_all("a"):
                    old_href = a_tag['href']
                    new_href = old_href.split("#")[-1]  # Get the fragment identifier only
                    ...
                    if sup_to_remove:
                        ...
                        elif sup_to_remove.text == 'dp':
                            a_tag['onclick'] = "opentab(event, 3,2)"
